...
| dcat:Distribution | healthdcatap:retentionPeriod     | resources[].retention_period     | interval  | Stored per-resource, not as a top-level dataset field: each entry in `resources` carries its own `retention_period`. This property makes use of the class dct:PeriodOfTime, in which a start and end date should be provided. |
